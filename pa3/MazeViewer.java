...
public class MazeViewer {
   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {
       ...
       int num_row = in.nextInt(); // the number of rows of maze
       int num_col = in.nextInt(); // the number of cols of maze

       // reading maze data....
       boolean[][] mazeData = new boolean[num_row][num_col];
       for(int row = 0; row < num_row; row++){
	   for(int col = 0; col < num_col + 1; col++){
	       int data = buffer.read();
	       if(data != 10){
		   // skip new line, 10 = nl
		   mazeData[row][col] = data == 49 ? true : false; // 49 = 1
	       }
	   }
       }
       ...
       line = buffer.readLine();
       in = new Scanner(line);
       int exit_row = in.nextInt();
       int exit_col = in.nextInt();

      return new MazeFrame(mazeData, new MazeCoord(start_row, start_col), new MazeCoord(exit_row, exit_col));
